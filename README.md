...
- **`NEXT.md`** — **read this before starting a work session on a project's config.** The actionable backlog: what is still open, per project, each item with its cost, its risk and the command that proves it done. `ARCHITECTURE.md` says *why*, `ROADMAP.md` says *what was done*, `NEXT.md` says *what to do next*.
